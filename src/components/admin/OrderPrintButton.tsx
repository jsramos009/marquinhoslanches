...
export function OrderPrintButton({ orderId }: { orderId: string }) {
  const enqueueFn = useServerFn(enqueueOrderPrintJob);
  ...
  const printMutation = useMutation({
    mutationFn: async () => {
      const queued = await enqueueFn({
        data: { orderId, requestKey: crypto.randomUUID() },
      });
      const claimed = await claimFn({ data: { jobId: queued.id, stationId } });
      if (!claimed) throw new Error("O trabalho de impressão já foi usado por outra aba.");
      ...
      try {
        ...
        try {
          ...
          console.error("[manual-print-claim-lost]", claimError);
        }
        throw error;
      } finally {
        setFallbackJob(null);
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1 rounded-lg border border-primary/40 bg-primary/10 px-2.5 py-1.5 text-xs font-semibold text-primary transition hover:bg-primary/20 disabled:opacity-50"
      ...
      {fallbackJob ? <KitchenTicket payload={fallbackJob.payload} /> : null}
    </>
  );
}
